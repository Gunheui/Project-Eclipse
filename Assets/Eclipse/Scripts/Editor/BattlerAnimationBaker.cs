...
namespace Eclipse.EditorTools
{
    public static class BattlerAnimationBaker
    {
        ...
        private const string AttackSuffix = "_Attack";
        ...
        private const float AttackFps = 24f;
        ...
        public static void Bake()
        {
            ...
            var clips = Units.ToDictionary(
                u => u.Folder,
                u => (Idle: BakeClip(u.Folder + IdleSuffix, LoadFrames(u.Folder, IdleSuffix), IdleFps, loop: true),
                    Attack: BakeClip(u.Folder + AttackSuffix, LoadFrames(u.Folder, AttackSuffix), AttackFps,
                        loop: false)));

            var shared = LoadOrCreateSharedController(clips[Units[0].Folder]);
            foreach (var unit in Units)
            {
            ...
            AssetDatabase.Refresh();
            Debug.Log($"[BattlerAnim] 클립 {clips.Count * 2}개 · 오버라이드 {clips.Count}개 · 아틀라스 완료");
        }
        ...
        /// <summary>
        /// 12종이 공유하는 컨트롤러. 상태는 대기·공격 둘이고, 배속은 런타임에서 animator.speed로 건다.
        /// </summary>
        /// <param name="baseClips">덮어쓸 자리를 잡아 줄 기준 모션. 유닛별 오버라이드가 이 자리를 갈아 끼운다.</param>
        private static AnimatorController LoadOrCreateSharedController(
            (AnimationClip Idle, AnimationClip Attack) baseClips)
        {
            var existing = AssetDatabase.LoadAssetAtPath<AnimatorController>(SharedControllerPath);
            ...
            idle.motion = baseClips.Idle;
            machine.defaultState = idle;

            var attack = machine.AddState(AttackState);
            attack.motion = baseClips.Attack;
        ...
        private static AnimatorOverrideController LoadOrCreateOverride(string folder, AnimatorController shared,
            (AnimationClip Idle, AnimationClip Attack) clips)
        {
            ...
            for (int i = 0; i < overrides.Count; i++)
            {
                ...
                var clip = slot.name.EndsWith(AttackSuffix) ? clips.Attack : clips.Idle;
