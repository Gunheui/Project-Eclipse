...
namespace Eclipse.View
{
    ...
    public class CurrencyHudView : MonoBehaviour
    {
        ...
        // 개발용 임시 재화 증감 버튼. 지급은 ICurrencyService 경유로 처리한다.
        [Header("더미 증감 버튼")]
        ...
        private void Start()
        {
            ...
            // ＋버튼 → 개발용 임시 재화 증감(구독 반응 관측용). 지급 권위는 ICurrencyService가 보유한다.
            essencePlus.OnClickAsObservable().Subscribe(_ => _currency.Grant(CurrencyType.Essence, 300)).AddTo(this);
            goldPlus.OnClickAsObservable().Subscribe(_ => _currency.Grant(CurrencyType.Gold, 5000)).AddTo(this);
            manualPlus.OnClickAsObservable().Subscribe(_ => _currency.Grant(CurrencyType.Manual, 1)).AddTo(this);
