namespace Eclipse.Domain
{
    ...
    /// </summary>
    public sealed class SeededRandom : IRandomService
    {
        // 상태 2워드(총 128비트). 매 난수마다 한 워드를 시프트로 섞고 두 워드를 더해 출력한다.
        private ulong _s0, _s1;

        /// <summary>
        /// 단일 정수 시드로 난수를 만든다. 시드를 splitmix64로 두 워드에 흩뿌려 상태를 채운다
        /// (서로 닮은 초기 상태·전부 0 상태를 피하려는 것 — 0이면 xorshift가 0만 뱉는다).
        /// </summary>
        ...
        /// <summary>
        /// [0, 1) 균등 실수. 64비트 결과의 상위 24비트만 2^24로 나눈다 — float 가수부가 23비트라
        /// 24비트를 넘으면 반올림 편향이 생기고, 상위비트를 쓰면 xorshift128+의 하위비트 선형성 약점도 피한다.
        /// </summary>
