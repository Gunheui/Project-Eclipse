...
{
    /// <summary>
    /// 파티 픽 화면의 상태. 보유 로스터를 셀로 노출하고, 탭 한 번으로 편성(PartyFormation)의 앵커 슬롯에 즉시
    ...
    public sealed class PartyPickViewModel : ViewModelBase
    {
        ...
        /// <summary> 로스터 셀 목록(보유 순서 고정). View가 한 번 순회해 셀을 생성한다. </summary>
        public IReadOnlyList<PartyPickItemViewModel> Items => _items;

        /// <summary> 역할 필터(null = 전체). View가 구독해 보이는 셀을 거른다. </summary>
        public ReactiveProperty<Role?> RoleFilter { get; } = new(null);

        /// <param name="save">보유 캐릭터 원천. 로스터 셀을 만든다.</param>
        /// <param name="spriteProvider">셀 초상 로딩 경로(공유 셀 VM에 전달).</param>
        ...
        public PartyPickViewModel(PlayerSave save, ISpriteProvider spriteProvider, PartyFormationViewModel formation)
        {
            ...
            _items = save.OwnedCharacters
                .Select(owned => new PartyPickItemViewModel(new CharacterCellViewModel(owned, spriteProvider)))
                .ToList();
        }

        /// <summary>
        /// 픽 세션을 시작한다. 각 셀의 슬롯 번호 배지를 현재 편성 상태로 시드하고 역할 필터를 전체로 초기화한다.
        /// 픽 화면이 전면에 설 때 호출한다.
        /// </summary>
        ...
        /// <summary>
        /// 탭된 셀을 앵커 슬롯(<see cref="PartyFormationViewModel.PickSlot"/>)에 배치한다. 그 캐릭터가 이미
        /// 앵커 슬롯에 있으면 재탭으로 보고 슬롯을 비운다. 다른 슬롯에 있었다면 그 슬롯이 비워지며 이동한다.
        /// View는 이 호출 뒤 화면을 닫아 편성으로 돌아간다.
        /// </summary>
        /// <param name="item">탭된 로스터 셀. null이면 아무 일도 하지 않는다.</param>
        ...
        /// <summary>역할 필터를 설정한다. null이면 전체 표시. View의 역할 버튼이 호출한다.</summary>
        /// <param name="role">보일 역할. null이면 필터 해제.</param>
        public void SetRoleFilter(Role? role) => RoleFilter.Value = role;

        // 각 셀의 배지를 현재 점유 슬롯 번호(1~4)로 다시 매긴다. 어느 슬롯에도 없으면 0.
