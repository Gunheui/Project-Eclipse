...
{
    /// <summary>
    /// 픽 화면 로스터 셀 하나를 감싸는 얇은 래퍼. 표시 데이터(초상·이름·등급·레벨)는 공유 셀 VM에 위임하고,
    /// 이 화면에만 있는 상태(점유 슬롯 번호)만 얹는다. 편성 상태를 공유 셀 VM에 넣지 않기 위한 경계다.
    /// </summary>
    public sealed class PartyPickItemViewModel : ViewModelBase
    {
        private readonly ReactiveProperty<int> _slotNumber = new(0);

        /// <summary> 표시 데이터(초상·이름·등급·레벨 로드)를 담당하는 공유 셀 VM. </summary>
        public CharacterCellViewModel Cell { get; }

        /// <summary> 이 셀이 표시하는 보유 캐릭터. 탭 시 편성 슬롯으로 배치된다. </summary>
        public OwnedCharacter Owned => Cell.Owned;

        /// <summary> 역할(정의에서 읽음). 역할 필터가 표시 여부를 가른다. </summary>
        public Role Role => Cell.Owned.Definition.role;
        ...
        public PartyPickItemViewModel(CharacterCellViewModel cell)
        {
            Cell = cell;
        }
