...
namespace Eclipse.Tests
{
    public class RuleBasedActionProviderTests
    {
        ...
        [Test]
        public void 아군_위급하면_준비된_힐스킬을_고른다()
        {
            ...
            var action = provider.Decide(actor, new List<ICombatant> { actor }, NoEnemies);

            Assert.AreSame(heal, action.Skill);
        }

        [Test]
        public void 아군_멀쩡하면_힐_대신_강한_공격_액티브를_고른다()
        {
            var basic = Runtime(Skill("b", 0, EffectType.Damage));
        ...
        [Test]
        public void 강한_액티브가_쿨이면_기본공격으로_폴백()
        {
            var basic = Runtime(Skill("b", 0, EffectType.Damage));
        ...
        [Test]
        public void 힐규칙_off면_위급해도_힐하지_않는다()
        {
            var basic = Runtime(Skill("b", 0, EffectType.Damage));
            var heal = Runtime(Skill("h", 0, EffectType.Heal));
