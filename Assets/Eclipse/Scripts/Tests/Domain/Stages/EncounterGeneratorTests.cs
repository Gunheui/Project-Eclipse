...
namespace Eclipse.Tests
{
    public class EncounterGeneratorTests
    {
        ...
        private sealed class FixedStageRandom : IStageRandom
        {
            private readonly int[] _values;
            private int _index;

            public FixedStageRandom(params int[] values) { _values = values; }
        ...
        private static StageTuningSO Tuning(params DepthPool[] depths)
        {
            var tuning = ScriptableObject.CreateInstance<StageTuningSO>();
        ...
        // 깊이 1만 검사하는 표적 테스트용. 나머지 깊이는 로드 검증의 4행 요건을 채우려고 넣는다.
        private static StageTuningSO FocusedTuning(int min, int max, float chance, params EnemySO[] pool)
        {
        ...
        private static StageTuningSO FourDepthTuning(float chance)
        {
        ...
        private static EncounterGenerator Seeded(StageTuningSO tuning, int stageSeed)
            => new EncounterGenerator(tuning,
                new SeededRandom(StageSeed.For(stageSeed, StageSeed.Stream.Encounter)),
                new SeededRandom(StageSeed.For(stageSeed, StageSeed.Stream.Mutation)));

        [Test]
        public void 같은_시드는_같은_조우를_낸다()
        {
            var first = Seeded(FourDepthTuning(0.45f), 20260727);
            var second = Seeded(FourDepthTuning(0.45f), 20260727);

            for (int depth = 1; depth <= 4; depth++)
                CollectionAssert.AreEqual(
                    Describe(first.Generate(depth, false)).ToList(),
                    Describe(second.Generate(depth, false)).ToList(),
                    "깊이 {0}에서 같은 시드인데 조우가 갈렸다", depth);
        }

        [Test]
        public void 변이_소비량이_달라져도_몹_선택은_그대로다()
        {
            var never = Seeded(FourDepthTuning(0f), 555);
            var always = Seeded(FourDepthTuning(1f), 555);
            ...
            for (int depth = 1; depth <= 4; depth++)
            {
                ...
                CollectionAssert.AreEqual(neverIds, alwaysIds,
                    "깊이 {0}에서 변이 스트림 소비가 조우 스트림을 밀었다", depth);
            }
        }
        ...
        {
            var generator = Seeded(FourDepthTuning(0.3f), 8899);
        ...
        [Test]
        public void 풀에서_고른_몹이_굴림_결과와_일치한다()
        {
            var a = Enemy("a");
            var b = Enemy("b");
            var c = Enemy("c");
            ...
            var generator = new EncounterGenerator(tuning,
                new FixedStageRandom(1, 2, 0), new FixedStageRandom(Miss));
        ...
        [Test]
        public void 변이는_마리마다_독립으로_굴린다()
        {
            var tuning = FocusedTuning(2, 2, 0.5f, Enemy("a"));
            // 1마리째: 적중 → 변이 인덱스 1 / 2마리째: 빗나감.
            var generator = new EncounterGenerator(tuning,
                new FixedStageRandom(0), new FixedStageRandom(Hit, 1, Miss));
        ...
        [Test]
        public void 변이가_적중하면_굴림_결과의_변이를_붙인다()
        {
            var tuning = FocusedTuning(1, 1, 0.5f, Enemy("a"));
            var generator = new EncounterGenerator(tuning,
                new FixedStageRandom(0), new FixedStageRandom(Hit, 2));
        ...
        public void 변이_확률이_0이어도_마리당_한_번_굴린다()
        {
            ...
            var mutationRng = new FixedStageRandom(Miss);
            new EncounterGenerator(tuning, new FixedStageRandom(0), mutationRng).Generate(1, false);
        ...
        public void 변이_확률이_100이면_마리당_적중_판정과_선택을_굴린다()
        {
            ...
            var mutationRng = new FixedStageRandom(9999, 0);
            new EncounterGenerator(tuning, new FixedStageRandom(0), mutationRng).Generate(1, false);
        ...
        public void 정예는_마리수_상한_고정에_전원_변이_정예다()
        {
            ...
            var encounterRng = new FixedStageRandom(0);
            var enemies = new EncounterGenerator(tuning, encounterRng, new FixedStageRandom(9999, 0))
        ...
        public void 보스_방은_변이도_정예도_없는_고정_편성이다()
        {
            ...
            var encounterRng = new FixedStageRandom(0);
            var mutationRng = new FixedStageRandom(0);
        ...
        {
            var generator = Seeded(FourDepthTuning(0f), 1);

            Assert.Throws<ArgumentOutOfRangeException>(() => generator.Generate(0, false));
            Assert.Throws<ArgumentOutOfRangeException>(() => generator.Generate(6, false));
        }

        private static void AssertInvalid(Action<StageTuningSO> corrupt)
        {
            var tuning = FourDepthTuning(0.3f);
            corrupt(tuning);
            Assert.Throws<ArgumentException>(
                () => new EncounterGenerator(tuning, new FixedStageRandom(0), new FixedStageRandom(0)));
        }

        [Test]
        public void 깊이가_빠지거나_겹치면_예외()
        {
            AssertInvalid(t => t.depths[3].depth = 3);                                  // 중복
            AssertInvalid(t => t.depths = Array.Empty<DepthPool>());
            AssertInvalid(t => t.depths = t.depths.Take(3).ToArray());                  // 보스 직전 깊이가 빔
            AssertInvalid(t => t.depths =                                               // 보스 깊이를 침범
                t.depths.Append(Depth(5, 3, 4, 0f, t.depths[0].allowedPool[0])).ToArray());
        }
