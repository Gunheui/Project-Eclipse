...
본문에서 다루지 않은 대안과 감수한 비용만 적었습니다.

| 결정 | 대안 | 고른 이유와 감수한 것 |
|---|---|---|
| VContainer | 수동 조립, Zenject | 씬 스코프 계층과 팩토리를 표준 문법으로 얻습니다. 대신 컴포지션 루트를 읽지 않으면 객체 그래프가 한눈에 안 들어옵니다 |
| MVVM + R3 | MVP, UniRx | View를 얇게 두고 폴링을 없앴습니다. 스트림 수명 관리라는 새 실수 지점이 생겨 구독 해지를 테스트로 지킵니다 |
| ATB 고정소수점 | float 게이지, 고정 턴제 | 실시간 게이지 감각을 유지하면서 부동소수점 차이를 배제했습니다. 게이지 산술이 장황해졌습니다 |
| 시드 스트림 분리 | 단일 스트림 | 튜닝이 다른 시스템의 수열을 밀지 않습니다. 시드 파생 규칙을 문서로 관리해야 합니다 |
| 챕터 상태를 전투 스코프에 | 별도 영속 저장 | 중단 복구를 만들지 않아도 기획 규칙이 성립합니다. 나중에 중간 저장을 넣으려면 소유권부터 다시 잡아야 합니다 |
| 자리 번호 보고 | 보상 값 보고 후 대조 | 위조 검증 자체가 필요 없어집니다. 화면과 상태기계가 제시물 인덱스로 결합됩니다 |
| 성장 서비스 3개 병렬 | 공통 트랜잭션 추상화 | 지금 규모에서는 중복이 추상화보다 쌉니다. 축이 더 늘면 재검토 대상입니다 |
| 세이브 마이그레이션 없음 | 버전별 필드 채움 | 반쪽 역직렬화를 원천 차단합니다. 배포 빌드로 가면 이 정책은 쓸 수 없습니다 |

원본 ADR과 학습 노트는 비공개 개인 문서로 관리합니다.
